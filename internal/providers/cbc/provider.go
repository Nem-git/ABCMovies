...
func (p *Provider) GetSeries(ctx context.Context, limit, offset int) ([]oas.Series, int, error) {
	...
	for _, r := range pr {
		items = append(items, mapSeries(r))
	}
...
func (p *Provider) GetSeriesByID(ctx context.Context, seriesID string) (*oas.Series, error) {
	...
	return mapSeriesFromShow(sr, seriesID)
}

func (p *Provider) GetSeriesPoster(ctx context.Context, seriesID string) (io.ReadCloser, error) {
	return p.fetchShowImage(ctx, seriesID, func(im *types.Images) string {
		if im != nil && im.Card != nil {
			return im.Card.URL
		}
		return ""
	})
...
func (p *Provider) GetSeasons(ctx context.Context, seriesID string, limit, offset int) ([]oas.Season, int, error) {
	...
	for _, lu := range lineups {
		seasons = append(seasons, mapSeason(lu, seriesID))
	}
	...
	for _, lu := range lineups {
		if lu.SeasonNumber == num {
			...
			for _, item := range lu.Items {
				eps = append(eps, mapEpisode(item))
			}
...
func (p *Provider) GetEpisodeThumbnail(ctx context.Context, seriesID, seasonID, episodeID string) (io.ReadCloser, error) {
	...
	for _, lu := range lineups {
		if lu.SeasonNumber == num {
			for _, item := range lu.Items {
				if strconv.Itoa(item.IDMedia) == episodeID && item.Images != nil && item.Images.Card != nil {
					return p.cli.getImage(ctx, item.Images.Card.URL)
				}
			}
		}
	}
...
func (p *Provider) GetMovieById(ctx context.Context, movieId string) (*oas.Movie, error) {
	...
	return mapMovieFromShow(sr, movieId)
}
...
func mapSeries(r types.Result) oas.Series {
	s := oas.Series{
	...
	if r.Images != nil && r.Images.Card != nil {
		if u, err := url.Parse(r.Images.Card.URL); err == nil {
			s.Poster = oas.NewOptURI(*u)
		}
	}
	return s
}

func mapSeriesFromShow(sr *types.ShowResponse, seriesID string) (*oas.Series, error) {
	for _, c := range sr.Contents {
		if c.Items != nil && c.Items.Results != nil {
			for _, r := range *c.Items.Results {
				if r.URL == seriesID {
	...
	// Fall back to primary show fields
	if sr.Title == "" {
		return nil, provider.ErrNotSupported
	}
	s := oas.Series{
	...
	if sr.Images != nil && sr.Images.Card != nil {
		if u, err := url.Parse(sr.Images.Card.URL); err == nil {
			s.Poster = oas.NewOptURI(*u)
		}
	}
	s.NumberOfSeasons = countLineups(sr)
...
func mapSeason(lu types.Lineup, seriesID string) oas.Season {
	s := oas.Season{
...
func mapEpisode(item types.Item) oas.Episode {
	e := oas.Episode{
...
func mapMovie(r types.Result) oas.Movie {
	m := oas.Movie{
...
func mapMovieFromShow(sr *types.ShowResponse, movieId string) (*oas.Movie, error) {
	m := oas.Movie{
		Type: oas.MovieTypeMovie,
		ID:   movieId,
		Name: sr.Title,
	...
	if sr.Images != nil {
		if sr.Images.Card != nil {
			if u, err := url.Parse(sr.Images.Card.URL); err == nil {
				m.Poster = oas.NewOptURI(*u)
			}
		} else if sr.Images.Background != nil {
			if u, err := url.Parse(sr.Images.Background.URL); err == nil {
				m.Poster = oas.NewOptURI(*u)
			}
		}
	}
...
func mapSearchResult(r types.Result) *oas.SearchResultItem {
...
func (p *Provider) fetchShowImage(ctx context.Context, showId string, selector func(*types.Images) string) (io.ReadCloser, error) {
	sr, err := p.cli.getShow(ctx, showId)
	if err != nil {
		return nil, err
	}

	// Takes the card image of the first piece of content in the show
	if sr.Contents[0].Lineups[0].Items[0].Images != nil {
