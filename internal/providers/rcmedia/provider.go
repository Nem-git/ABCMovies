...
func New(cfg Config) *Provider {
	...
	if cfg.SeriesCategoryID == "" {
		cfg.SeriesCategoryID = "shows"
	}
	if cfg.MovieCategoryID == "" {
		cfg.MovieCategoryID = "all-films"
	}
	return &Provider{
		cfg: cfg,
		cli: newClient(cfg.AppCode, opts),
	}
}
...
func (p *Provider) GetSeries(ctx context.Context, limit, offset int) ([]oas.Series, int, error) {
	f, err := p.cli.getCategory(ctx, p.cfg.SeriesCategoryID, pageFor(offset, limit), pageSizeFor(limit))
	if err != nil {
...
func (p *Provider) GetMovies(ctx context.Context, limit, offset int) ([]oas.Movie, int, error) {
	f, err := p.cli.getCategory(ctx, p.cfg.MovieCategoryID, pageFor(offset, limit), pageSizeFor(limit))
	if err != nil {
