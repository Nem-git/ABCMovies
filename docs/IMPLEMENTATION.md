...
| M3 | Enrichment (catalogue slot) | §2.4, §5.2 | Enrichment off by default; structured TitleMetadata with per-field provenance and external-ID-to-record lookup; field-level merging across catalogue slots and provider adapters; provenance retained per external ID; heuristic-resolved IDs require corroboration before driving a merge (§5.3) |
